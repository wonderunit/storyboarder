...
module.exports = function( THREE ){
  ...
  RoundedBoxGeometry = function (
    ...
    var uvs = 		new THREE.BufferAttribute( new Float32Array( totalVertexCount * 2 ), 2 );
    ...
    function getMappedUV(position, normal) {
      var plane = new THREE.Plane(normal);
      var target = new THREE.Vector3();
      
      plane.projectPoint(position, target);
    ...
    for ( var i = 0 ; i < vertexPool.length ; i ++ ){
      ...
      var uv = getMappedUV(vertexPool[i], normalPool[i])
      uvs.setXY(
        index,
        uv[0],
        uv[1]
