...
const HairInspector = connect(
  ...
  React.memo(
    ({ id, withState, createObject, selectAttachable, deleteObjects }) => {
      ...
      const getSelectedHair = () => {
        ...
        let object = Object.values(sceneObjects).find(
          (item) =>
      ...
      const findModelFromObject = (object) => {
        ...
        withState((dispatch, state) => {
          ...
          model = Object.values(allModels).find(
            (item) => item.id === object.model
          )
        })
        return model
      }
      ...
      const models = useMemo(() => {
        ...
        withState((dispatch, state) => {
          let allModels = state.models
          attachableModels = Object.values(allModels).filter(
            (m) => m.type === 'attachable' && m.attachableType === 'hair'
          )
        })

        sortedAttachables.current = attachableModels.map(
          (attachable, index) => {
            return {
              value: [attachable.name, attachable.keywords]
                .filter(Boolean)
                .join(' '),
              id: index
            }
        ...
        <React.Fragment>
          ...
          <div className="thumbnail-search column">
            <div className="row" style={{ padding: '6px 0' }}>
              <SearchList
                label="Search models …"
                list={sortedAttachables.current}
              ...
              <FileInput
                value={isCustom ? selectValue() : 'Select File …'}
                ...
                itemData={{
                  ...
                  selectedFunc: (item) => {
                    return (
                      ...
                      selectedModel.current.id === item.id
                    )
                  },
                  selectInitial: true
                }}
