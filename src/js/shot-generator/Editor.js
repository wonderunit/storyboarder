...
const RemoteInputView = ({ remoteInput }) => {
    ...
    return h(
        ['div#remoteInputView', ['div',
            'input', ['div', 'accel: ' + accel],
            ['div', 'mag: ' + mag],
            ['div', 'sensor: ' + sensor],
            ['div', 'down: ' + down]
        ]],
                ...
                [
                    ...
                    [
                        ...
                        [
                            ...
                            Object.values(characterPresets).map(preset => ['option', { value: preset.id }, preset.name])
                        ]
                    ]
                ],
            ...
            ['div.row', { style: { margin: '9px 0 6px 0', paddingRight: 0 } },
                [
                    ...
                    [
                        ...
                        [
                            ['option', { value: '', disabled: true }, '---'],
                            sortedPosePresets.map(preset => ['option', { value: preset.id }, preset.name])
                        ]
                    ]
                ],
...
const InspectedElement = ({ sceneObject, models, updateObject, selectedBone, machineState, transition, selectBone, updateCharacterSkeleton, storyboarderFilePath }) => {
    const createOnSetValue = (id, name, transform = value => value) => value => updateObject(id, {
    ...
    let volumeSliders = (sceneObject.model === 'box') ? [
        ...
        NumberSlider, {
            ...
            onSetValue: value => updateObject(
                sceneObject.id, { width: value, height: value, depth: value }
            )
        }
    ]
    ...
    const putSphereHere = (coords, padre, color) => {
        ...
        sobj.position.set(coords.x, coords.z, -coords.y) //.multiplyScalar(-100)
            //console.log( 'addng sphere obj:', sobj )
        padre.add(sobj)
    }

    const putSphereHere2 = (coords, padre, color, negX, negY, negZ) => {
        let sphge = new THREE.SphereBufferGeometry(0.01, 6, 6)
        let sphma = new THREE.MeshStandardMaterial({ color: color })
        let sphme = new THREE.Mesh(sphge, sphma)
        let sobj = new THREE.Object3D().add(sphme)
        sobj.position.set(negX ? -coords.x : coords.x, negY ? -coords.y : coords.y, negZ ? -coords.z : coords.z) //.multiplyScalar(-100)
        console.log('punem vert la: ', sobj.position)
        padre.add(sobj)
    }
    ...
    const clcLowestVertInObject = (geometry, padre) => {
        ...
        let prentMatrix = padre.matrixWorld.clone()
        let zeroPoint = new THREE.Vector3(0, 0, 0).applyMatrix4(prentMatrix)

        let distToZero
        for (i = 0; i <= position.count; i++) {
            vertex.fromBufferAttribute(position, i)
                //vertex.applyMatrix4(inversedMatrix)
            vertex.applyMatrix4(prentMatrix)
            let zp = vertex.clone()
            zp.y = zeroPoint.y
            ...
            if (vertex.distanceTo(zeroed) < lowest) {
                distToZero = vertex.distanceTo(zp)
                lowest = vertex.distanceTo(zeroed)
                lowestVert = vertex.clone()
            }
        }

        //console.log('lowest: ', lowestVert, lowest, zeroPoint)
        //putSphereHere2(lowestVert.clone(), scene, '0x00ff00')
        return {
            lowestVert,
            lowest: lowest / 100,
            distToZero
    ...
    const calcLowestVert = (skinnedMesh, padre) => {
        ...
        if (index !== null) {
            ...
            for (i = 0; i < index.count; i++) {
                vertex.fromBufferAttribute(position, i) //.applyMatrix4(inverdsedMatrix)
    ...
    const dropObject = () => {
        ...
        if (sceneObject.type == 'character') {
            ...
            updateObject(sceneObject.id, { z: lowest.distToZero })
        }
    }
    ...
    return h([
        ...
        [
            LabelInput,
            {
                ...
                label: sceneObject.name != null ?
                    sceneObject.name : sceneObject.displayName,
                onFocus,
                onBlur,
                setLabel: name => {
        ...
        sceneObject.type == 'light' && [
            [
                'div.column', [NumberSlider, { label: 'intensity', value: sceneObject.intensity, min: 0.025, max: 1, onSetValue: createOnSetValue(sceneObject.id, 'intensity') }],
            ],
            [
                'div.column', [NumberSlider, {
                    label: 'angle',
            ...
            [
                'div.column', [NumberSlider, { label: 'distance', value: sceneObject.distance, min: 0.025, max: 100, onSetValue: createOnSetValue(sceneObject.id, 'distance') }],
            ],
            [
                'div.column', [NumberSlider, { label: 'penumbra', value: sceneObject.penumbra, min: 0, max: 1, onSetValue: createOnSetValue(sceneObject.id, 'penumbra') }],
            ],
            [
                'div.column', [NumberSlider, { label: 'decay', value: sceneObject.decay, min: 1, max: 2, onSetValue: createOnSetValue(sceneObject.id, 'decay') }],
            ],
        ],
        ...
        sceneObject.type == 'camera' && ['div', [NumberSlider, {
            label: 'roll',
            min: -45,
            max: 45,
            step: 1,
        ...
        (sceneObject.type == 'camera' || sceneObject.type == 'light') && ['div', [NumberSlider, {
            label: 'tilt',
...
const Element = React.memo(({ index, style, sceneObject, isSelected, isActive, selectObject, updateObject, deleteObject, setActiveCamera, machineState, transition, allowDelete }) => {
    ...
    return h([
        ...
        [
            ...
            ['div.row', [
                isActive ? ['span.active', [Icon, { src: 'icon-item-active' }]] : [],

                sceneObject.type === 'camera' ? [] :
                sceneObject.visible ?
                isSelected ? ['a.visibility[href=#]', { onClick: onToggleVisibleClick },
                    [Icon, { src: 'icon-item-visible' }]
                ] : [] : ['a.visibility[href=#]', { onClick: onToggleVisibleClick },
                    [Icon, { src: 'icon-item-hidden' }]
                ],

                allowDelete ? ['a.delete[href=#]', { onClick: onDeleteClick }, 'X'] : ['a.delete', { style: { opacity: 0.1 } }, 'X']
            ]]
        ]
    ])
})
    ...
    return h(
        ['div#toolbar', { key: 'toolbar' },
            ['div.toolbar__addition.row', [
                ['a[href=#]', { onClick: preventDefault(onCreateCameraClick) },
                    [
                        [Icon, { src: 'icon-toolbar-camera' }], 'Camera'
                    ]
                ],
                ['a[href=#]', { onClick: preventDefault(onCreateObjectClick) },
                    [
                        [Icon, { src: 'icon-toolbar-object' }], 'Object'
                    ]
                ],
                ['a[href=#]', { onClick: preventDefault(onCreateCharacterClick) },
                    [
                        [Icon, { src: 'icon-toolbar-character' }], 'Character'
                    ]
                ],
                ['a[href=#]', { onClick: preventDefault(onCreateLightClick) },
                    [
                        [Icon, { src: 'icon-toolbar-light' }], 'Light'
                    ]
                ],
                ['a[href=#]', { onClick: preventDefault(onCreateVolumeClick) },
                    [
                        [Icon, { src: 'icon-toolbar-volume' }], 'Volume'
                    ]
                ],
            ]],
            ...
            ['div.toolbar__board-actions.row', [
                ['a[href=#]', { onClick: preventDefault(onSaveToBoardClick) },
                    [
                        [Icon, { src: 'icon-toolbar-save-to-board' }], 'Save to Board'
                    ]
                ],
                ['a[href=#]', { onClick: preventDefault(onInsertNewBoardClick) },
                    [
                        [Icon, { src: 'icon-toolbar-insert-as-new-board' }], 'Insert As New Board'
                    ]
                ],
            ]]
        ]
        ...
        return h(
            ['div.camera-inspector',
                ...
                ['div.row',
                    ...
                    [
                        'div',
                        `${cameraName}, ${Math.round(focalLength)}mm, f/1.4`, ['br'],
                        `Height: ${feetAndInchesAsString(heightFeet, heightInches)} Tilt: ${tiltInDegrees}°`, ['br'],
...
const GuidesInspector = connect(
    ...
    (({
        ...
        h(['div.guides-inspector', [
            'div.row', ['div.guides-inspector__label', 'Guides'],
...
const CamerasInspector = connect(
    ...
    ({
        ...
        return h(['div.cameras-inspector', [
            'div.row', ['div.cameras-inspector__label', 'Camera'],
            ['div.round-buttons-panel',
                _cameras.map(
                    (camera, n) => [
                        'a[href=#]',
...
const Editor = connect(
    ...
    ({ mainViewCamera, createObject, selectObject, updateModels, loadScene, saveScene, activeCamera, setActiveCamera, resetScene, remoteInput, aspectRatio, sceneObjects, selection, selectedBone, onBeforeUnload, setMainViewCamera, withState, attachments }) => {
        ...
        return React.createElement(
            SceneContext.Provider, { value: { scene: scene.current } },
            h(
                ['div.column', { style: { width: '100%' } },
                    [
                        ...
                        ['div.row', { style: { flex: 1 } },
                            ...
                            ['div.column.fill', ['div#camera-view', { ref: mainViewContainerRef, style: { paddingTop: `${(1 / aspectRatio) * 100}%` } },
                                    // camera canvas
    ...
    ({ presets, loadScenePreset, createScenePreset, updateScenePreset, deleteScenePreset, transition }) => {
        ...
        return h([
            ...
            [
                ...
                ['ul', Object.values(presets.scenes).map(preset => ['li.element', { style: { display: 'flex', justifyContent: 'space-between' } },
                        ...
                        [
                            LabelInput,
                            {
                                ...
                                label: preset.name != null ?
                                    preset.name : `Preset ${shortId(preset.id)}`,
                                onFocus,
                                onBlur,
                                setLabel: name => {
