...
//        group.rotateY(Math.random()*4)
    ...
    createScene({
      store,
      label: 'difference applied to a specific bone in an object [rotation: 0,0,0 | camera: 90,0,0 ]',
      setup: ({ store, scene, camera, clock, renderer }) => {
        let group = new THREE.Object3D()
       // let suzanne = createSuzanne()
        //group.add(suzanne)
...
//        group.rotateY(Math.random()*4)
        ...
        let animate = () => {
          ...
          if (remoteInput.down) {
            ...
            let parentParentOffset = new THREE.Quaternion().clone().inverse().multiply(targetobject.parent.parent.quaternion.clone())

            let objectQuaternion = startingObjectQuaternion.clone().inverse()

            objectQuaternion.multiply(startingObjectOffset)
            objectQuaternion.multiply(parentOffset.inverse())
            objectQuaternion.multiply(parentParentOffset.inverse())
           objectQuaternion.multiply(cameraOffset)
    ...
    createScene({
      store,
      label: 'difference applied to a specific bone in an object [rotation: 90,0,0 | camera: 0,0,0 ]',
      setup: ({ store, scene, camera, clock, renderer }) => {
        let group = new THREE.Object3D()
       // let suzanne = createSuzanne()
        //group.add(suzanne)
        ...
        let animate = () => {
          ...
          if (!remoteInput.down) {
           ...
           if (remoteInput.down && !down) {
            ...
            targetobject = snake.children[0].children[2].skeleton.bones[9]
    ...
    createScene({
      store,
      label: 'difference applied to a specific random bone in an object [rotation: random | camera: random ]',
      setup: ({ store, scene, camera, clock, renderer }) => {
        let group = new THREE.Object3D()
        let snake = createSnake()
        group.add(snake)
        ...
        let campos = new THREE.Vector3(Math.random()*2-1, Math.random()*2-1, Math.random()*2-1).normalize().multiplyScalar(2)
        camera.position.x = campos.x*4
        camera.position.z = campos.y*4
        camera.position.y = campos.z*4
        camera.lookAt(0, 0, 0)
